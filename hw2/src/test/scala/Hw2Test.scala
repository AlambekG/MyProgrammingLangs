...
class Hw2Test extends org.scalatest.funsuite.AnyFunSuite {/*
  PCall(LetRec(Var(f),Var(x),
  Ite(Iszero(Var(x)),Const(0),Add(Var(x),Paren(PCall(Var(f),Sub(Var(x),Const(1))))))
  ,Var(f)),
  Const(1))*/
  println(LetRecParserDriver("letrec f(x) = if iszero x then 0 else x + (f x-1) in f 1"))
